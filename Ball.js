...
Ball.prototype.handleBallInPocket = function(){
    ...
    let inPocket = CONSTANTS.pockets.some(pocket => {//pocketed
...
Ball.prototype.collideWithTable = function(table){
    ...
    if(this.position.y <= table.TopY + CONSTANTS.ballRadius){//top wall 
        this.position.y = table.TopY + CONSTANTS.ballRadius;
        this.velocity = new Vector2(this.velocity.x, -this.velocity.y);
        collided = true;
    }

    if(this.position.x >= table.RightX - CONSTANTS.ballRadius){//right wall
        this.position.x = table.RightX - CONSTANTS.ballRadius;
        this.velocity = new Vector2(-this.velocity.x, this.velocity.y);
        collided = true;
    }

    if(this.position.y >= table.BottomY - CONSTANTS.ballRadius){//bottom wall
        this.position.y = table.BottomY - CONSTANTS.ballRadius;
        this.velocity = new Vector2(this.velocity.x, -this.velocity.y);
        collided = true;
    }

    if(this.position.x <= table.LeftX + CONSTANTS.ballRadius){//left wall
    ...
    if(collided){
        this.velocity = this.velocity.mult(1 - CONSTANTS.collisionEnergyLoss);//energy loss
    }
